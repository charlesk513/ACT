...
public class ScapegoatTree<Key extends Comparable<Key>, Value> {
    ...
    // kc: helper to get size of a node, returns 0 if null
    ...
    /*
     * kc: This method inserts a key-value pair into the subtree rooted at 'node'.
     *     Steps:
     *       1. If node is null, create a new leaf node.
     *       2. Compare key with node.key to decide left or right.
     *       3. After inserting, update size and height.
     *       4. If the tree is too tall (height > alpha * log2(size)), rebuild.
     */
    private Node put(Node node, Key key, Value val) {
        // TODO: fix the placeholders for left, right, equal cases below
        ...
        if (cmp < 0) {
            // kc: key is smaller, go left
            node.left = put(node.left, key, val);
        } else if (cmp > 0) {
            // kc: key is bigger, go right
            node.right = put(node.right, key, val);
        } else {
            // kc: key already exists, just update the value
            node.val = val;
        }

        // kc: update size and height after insertion
        node.size = 1 + size(node.left) + size(node.right);
        node.height = 1 + Math.max(height(node.left), height(node.right));

        // kc: check if this subtree violates the scapegoat balance condition
        // if height exceeds alpha * log2(size), rebuild the subtree
        if (node.height > alpha * log2(node.size)) {
            node = rebuild(node); // kc: rebalance by rebuilding
        }
    ...
    /*
     * kc: Rebuild a tree to make it perfectly balanced.
     *     Steps:
     *       1. Collect all nodes via in-order traversal into a list.
     *       2. Build a balanced BST from that list using the middle element as root.
     */
    ...
    /*
     * kc: Perform in-order traversal (left -> root -> right) and collect
     *     all nodes into the 'nodes' list. This gives us sorted order.
     */
    ...
    private void inorder(Node node, ArrayList<Node> nodes) {
        ...
        if (node == null) return; // kc: base case, nothing to add

        inorder(node.left, nodes);  // kc: visit left subtree first
        nodes.add(node);            // kc: visit current node (adds in sorted order)
        inorder(node.right, nodes); // kc: visit right subtree last
    }

    /*
     * kc: Build a balanced BST from a sorted subarray nodes[lo..hi].
     *     Uses divide and conquer:
     *       - Pick the middle element as root.
     *       - Recursively build left and right subtrees from the two halves.
     *       - Update size and height of the new root node.
     */
    ...
    private Node balanceNodes(ArrayList<Node> nodes, int lo, int hi) {
        ...
        // kc: (1a) build balanced BST from left half
        Node leftSubtree = balanceNodes(nodes, lo, mid - 1);

        // kc: (1b) build balanced BST from right half
        Node rightSubtree = balanceNodes(nodes, mid + 1, hi);

        // kc: (2) use the middle node as root
        Node root = nodes.get(mid);

        // kc: (3) attach left and right children
        root.left = leftSubtree;
        root.right = rightSubtree;

        // kc: (4) update size = number of nodes in this subtree
        root.size = 1 + size(root.left) + size(root.right);

        // kc: (4) update height = 1 + max height of children
        root.height = 1 + Math.max(height(root.left), height(root.right));

        // kc: (5) return the balanced root
        return root;
    }
